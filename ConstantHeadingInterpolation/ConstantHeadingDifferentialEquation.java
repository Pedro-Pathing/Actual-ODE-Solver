...
public class ConstantHeadingDifferentialEquation implements FirstOrderDifferentialEquations {
    ...
    public void computeDerivatives(double v, double[] doubles, double[] doubles1) throws MaxCountExceededException, DimensionMismatchException {
        ...
        double scalar = (pathPrimes.dotProduct(new Vector2D(FastMath.cos(theta), FastMath.sin(theta))) * v_max - pathPrimes.getNorm() * ((mu_k * mass * 9.81) + c_1 * FastMath.abs(FastMath.sin(theta-FastMath.atan2(pathPrime.getY(), pathPrime.getX())))))/pathPrimes.getNormSq();
